...
def non_max_suppression_multilands(prediction, conf_thres=0.5, nms_thres=0.4, landmarks=1):
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        if not image_pred.size(0):
            continue
        # Object confidence times class confidence
        score = image_pred[:, 4] * image_pred[:, 5:-landmarks].max(1)[0]
        # Sort by it
        image_pred = image_pred[(-score).argsort()]
        class_confs, class_preds = image_pred[:, 5:-landmarks].max(1, keepdim=True)
        detections = torch.cat(
            (image_pred[:, :5], image_pred[:, -landmarks:], class_confs.float(), class_preds.float()), 1)
        # Perform non-maximum suppression
        keep_boxes = []
        while detections.size(0):
            large_overlap = bbox_iou(
            ...
            # Merge overlapping bboxes by order of confidence
            detections[0, :4] = (weights * detections[invalid, :4]).sum(0) / weights.sum()
            detections[0, 5:5+landmarks] = (weights * detections[invalid, 5:5+landmarks]).sum(0) / weights.sum()
            keep_boxes += [detections[0]]
            detections = detections[~invalid]
        if keep_boxes:
            output[image_i] = torch.stack(keep_boxes)
...
def post_process_expected(pred_boxes, expected=1):
    for b, boxes in enumerate(pred_boxes):
        if boxes is not None:
            nboxes = torch.FloatTensor(expected, boxes.size(1)).fill_(0)
            for i in range(len(boxes)-1, -1, -1):
                obj_class = min(int(boxes[i, -1]), expected-1)
                nboxes[obj_class] = boxes[i]
            # nboxes = torch.maximum(nboxes, 0) # remove any less than 0's
            pred_boxes[b] = nboxes
    return pred_boxes
