...
def rescale_boxes(boxes, current_dim, original_shape):
    ...
    if boxes.shape[-1] > 7:
        boxes[:, 5] = ((boxes[:, 5] - pad_x // 2) / unpad_w) * orig_w
        boxes[:, 6] = ((boxes[:, 6] - pad_y // 2) / unpad_h) * orig_h
    return boxes
...
def get_land_statistics(outputs, targets):
    ...
    for sample_i in range(len(outputs)):
        ...
        if outputs[sample_i].shape[-1] > 7:
            plands = outputs[sample_i][:, 5:7]
            tlands = targets[targets[:, 0] == sample_i][:, -2:]
        else:
...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres, isLandmark=False):
    ByteTensor = torch.cuda.ByteTensor if pred_boxes.is_cuda else torch.ByteTensor
    FloatTensor = torch.cuda.FloatTensor if pred_boxes.is_cuda else torch.FloatTensor
...
def fake_target_build(target, img_size):
    ...
    gxl, gyl = gxyl.t()
    # Convert to position relative to box
    gxy = target[:, 2:4] * img_size
    gwh = target[:, 4:6] * img_size
    gx, gy = gxy.t()
    gw, gh = gwh.t()
    gi, gj = gxy.long().t()
